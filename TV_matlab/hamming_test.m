...
beta = 1;  % Higher beta for stronger noise suppression 
N1_wd = N1 - 32*reso;
N2_wd = N2 - 32*reso;
...
for sub_num = 3:length(dirname)
        ...
        ssim_ = ssim_ + ssim(recon/pixelscale,gt/pixelscale,'DynamicRange', 1);
        nrmse_ = nrmse_ + norm(recon(:)-gt(:))/l2scale;

        end
        disp([ssim_/count,nrmse_/count]);
end