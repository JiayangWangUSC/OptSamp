...
snr = 10
reso = 0
print('non-uniform w. uniform-init')
...
class Sample(torch.nn.Module): 
    ...
    def __init__(self,sigma,factor):
        super().__init__()
        self.weight = 2 * factor*N1/(N1-32*reso)*N2/(N2-32*reso) * torch.ones((N2-32*reso) / 2)
        self.sigma = sigma

    def forward(self,kspace):

        filled_weight = torch.zeros(N2-32*reso)
        filled_weight[::2] = self.weight
        mask =  torch.zeros((N1,N2))
        mask[(16*reso):(N1-16*reso),(16*reso):(N2-16*reso)] =  1.0 / (filled_weight ** 0.5).unsqueeze(0).repeat(N1-32*reso,1)
...
#    print("normalized train loss:",trainloss_normalized/331/8," normalized val loss:",valloss_normalized/42/8, flush = True)
    ...
    torch.save(recon_model,"/project/jhaldar_118/jiayangw/OptSamp/model/uni_init_mse_snr"+str(snr)+"_reso"+str(reso))
    torch.save(weight,"/project/jhaldar_118/jiayangw/OptSamp/model/uni_init_mse_mask_snr"+str(snr)+"_reso"+str(reso))
